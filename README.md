...
- **Alignment** — each event can be anchored left, center, or right relative to its date
- **Notes** — optional multi-line description that can be shown directly on the timeline
...
- **Today marker** — a red dashed vertical line marks the current date; can be toggled on/off from the toolbar
- **Smooth navigation** — pan by dragging, zoom with the scroll wheel, or use the toolbar buttons
- **Resizable timeline area** — drag the divider below the timeline to adjust its height
